...
module MastermindVGA (
    ...
    logic [7:0] backg_red, backg_green, backg_blue;
    assign {VGA_R, VGA_G, VGA_B} = ((color != BLACK) ? color : 
                                    {backg_red, backg_green, backg_blue});

    //background gradient
    always_comb begin
        if (y < 10'd150) begin
            // dark navy
            backg_red   = (8'd5+ (y>>3) > 8'd255) ?
                        8'd255: 8'd5  + (y>>3);
            backg_green = 8'd2;
            backg_blue  = (8'd120 + (y>>3) > 8'd255) ?
                        8'd255: 8'd120 + (y>>3);
        end
        else if (y < 10'd300) begin
            // nave to blueish purple
            backg_red   = (8'd23+ ((y- 10'd150)>>2)> 8'd255)   ?
                        8'd255: 8'd23 + ((y - 10'd150)>>2);
            backg_green = 8'd2;
            backg_blue  = (8'd138- ((y- 10'd150)>> 4) > 8'd255) ?
                        8'd255: 8'd138- ((y- 10'd150)>> 4);
        end
        else if (y < 10'd450) begin
            // blueish purple into deep purple
            backg_red   = (8'd60 + ((y- 10'd300)>>1) > 8'd255) ?
                        8'd255: 8'd60+ ((y- 10'd300)>>1);
            backg_green = 8'd2;
            backg_blue  = (8'd128 + ((y - 10'd300)>>4) > 8'd255) ?
                        8'd255: 8'd128+ ((y- 10'd300)>>4);
        end
        else begin
            //purple to magenta
            backg_red   = (8'd135+ ((y - 10'd450)>>1) > 8'd255) ?
                        8'd255: 8'd135+ ((y - 10'd450)>>1);
            backg_green = 8'd2;
            backg_blue  = (8'd137- ((y - 10'd450)>>2) < 8'd0)   ?
                        8'd0: 8'd137- ((y - 10'd450)>>2);
        end
    end
...
module vga (
    ...
    //     count of   0 - 599 is display time (thus row_count is correct here)
    //     count of 600 is front porch
    //     count of 601 - 604 is VS=0 pulse width
    //     count of 605 - 627 is back porch
    //_____________________________________________
    ...
    assign row_clear  = (row_count >= 10'd627);
    assign row_enable = (col_count == 11'd1055);
    assign VS         = (row_count < 10'd601) | (row_count > 10'd604);
    assign v_blank    = (row_count >= 10'd600);

    //_____________________________________________  
    // Col counter counts from 0 to 1055
    //     count of    0 - 799 is display time (col is div by 2)
    //     count of 800 - 839 is front porch
    //     count of 840 - 967 is HS=0 pulse width
    //     count of 968 - 1055 is back porch
    //_____________________________________________
    ...
    assign col_clear  = (col_count >= 11'd1055);
    assign col_enable = 1'b1;
    assign HS         = (col_count < 11'd840) | (col_count > 11'd967);
    assign h_blank    = col_count > 11'd799;
